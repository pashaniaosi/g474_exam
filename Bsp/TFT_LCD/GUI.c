...
{  
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80) Gui_DrawFont_GBK16(x+(size/2)*t,y,RED,GRAY0,(uint8_t*)('0'));
				else Gui_DrawFont_GBK16(x+(size/2)*t,y,RED,GRAY0,(uint8_t*)(' '));
 				continue;
			}else enshow=1; 
		 	 
		}
		Gui_DrawFont_GBK16(x+(size/2)*t,y,RED,GRAY0,(uint8_t*)(temp+'0'));
	}
} 
