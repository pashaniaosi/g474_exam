...
void RTC_Init_AlarmA(uint8_t hour, uint8_t minute, uint8_t second)
{
  ...
  rtc_alarm.AlarmTime.SubSeconds = 0;
  rtc_alarm.AlarmMask = RTC_ALARMMASK_DATEWEEKDAY|RTC_ALARMMASK_HOURS
                              |RTC_ALARMMASK_MINUTES;
  rtc_alarm.AlarmSubSecondMask = RTC_ALARMSUBSECONDMASK_ALL;
  rtc_alarm.AlarmDateWeekDaySel = RTC_ALARMDATEWEEKDAYSEL_DATE;
  rtc_alarm.AlarmDateWeekDay = 1;
  rtc_alarm.Alarm = RTC_ALARM_A;
  if (HAL_RTC_SetAlarm_IT(&hrtc, &rtc_alarm, RTC_FORMAT_BIN) != HAL_OK) // 重新设置定时中断
  {
    Error_Handler();
  }
}
